...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    if tv_movie=='movie':
      ...
      if Addon.getSetting('debrid_select')=='0' :
        search_url=[clean_name(original_title,1).replace(' ','%20')+'%20s'+season_n+'e'+episode_n,clean_name(original_title,1).replace(' ','%20')+'%20s'+season_n]
      else:
    ...
    for itt in search_url:
        ...
        x=requests.get('https://thekat.app/usearch/{0}%20category:{1}/#'.format(itt,s_type),headers=base_header,timeout=10).content
        logging.warning('https://thekat.app/usearch/{0}%20category:{1}/#'.format(itt,s_type))
