...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    if tv_movie=='movie':
     ...
     if Addon.getSetting('debrid_select')=='0' :
        search_url=[('%s s%se%s'%(clean_name(original_title,1),season_n,episode_n)).lower(),('%s s%s'%(clean_name(original_title,1),season_n)).lower()]
     else:
    ...
    for itt in search_url:
      for page in range(0,4):
        ...
        logging.warning('https://limetorrents.at/search?category=%s&search=%s'%(search_type,itt))
