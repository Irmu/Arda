...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    if tv_movie=='movie':
     search_url=[('%s-%s'%(clean_name(original_title,1).replace(' ','-'),show_original_year)).lower()]
      ...
      if Addon.getSetting('debrid_select')=='0' :
        search_url=[('%s-s%se%s'%(clean_name(original_title,1).replace(' ','-'),season_n,episode_n)).lower(),('%s-s%s'%(clean_name(original_title,1).replace(' ','-'),season_n)).lower()]
      else:
    ...
    logging.warning('http://www.magnetdl.com/search/?q=%s&m=%s'%(search_url,match))
