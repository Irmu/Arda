...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    if tv_movie=='movie':
      ...
      if Addon.getSetting('debrid_select')=='0' :
         search_url=[clean_name(original_title,1).replace(' ','+')+'+s'+season_n+'e'+episode_n,clean_name(original_title,1).replace(' ','+')+'+s'+season_n]
      else:
    ...
    for itt in search_url:
     for page in range(1,4):
        ...
        x,cook=cloudflare_request('https://btdb.io/search/%s/?sort=popular&page=%s'%(itt,str(page)))
