...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    if tv_movie=='tv':
        if Addon.getSetting('debrid_select')=='0' :
            query=[clean_name(original_title,1).replace(' ','+')+'+s%s'%(season_n),clean_name(original_title,1).replace(' ','+')+'+s%se%s'%(season_n,episode_n)]
        else:
    ...
    for qrr in query:
        ...
        for el in response['content']:
            ...
            if tv_movie=='tv':
                ...
                if ('s%se%s.'%(season_n,episode_n) not in el['name'].lower()) and ('s%se%s '%(season_n,episode_n) not in el['name'].lower()):
                    if ('s%s'%(season_n) not in el['name'].lower()) and ('season.%s'%season_n not  in el['name'].lower()) and ('season %s'%season_n not  in el['name'].lower()):
                        continue
            ...
            info=(PTN.parse(el['name'].replace(' ','.').replace('(','').replace(')','')))
            check=True
            check1=False
            '''
            try:
                ttile=info['title'].replace('-','.').replace('_','.').replace(' ','.')
                logging.warning(ttile)
                if ttile.lower().replace('-','.').replace('_','.').replace(' ','.')==clean_name(original_title,1).lower().replace('-','.').replace('_','.').replace(' ','.'):
                    check=True
            except:
                ttile=clean_name(original_title,1).lower()
                if clean_name(original_title,1).lower().replace('-','.').replace('_','.').replace(' ','.') in ttile.lower().replace('-','.').replace('_','.').replace(' ','.'):
                    check=True
            '''
            try:
                tyear=info['year']
