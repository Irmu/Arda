...
if action == None:
...
elif action == "findKeywordsMovies":
    from resources.lib.indexers import movies

    movies.movies().findKeywords(url)

elif action == "findKeywordsTV":
    from resources.lib.indexers import tvshows

    tvshows.tvshows().findKeywords(url)
    ...
    try:
        downloader.download(
            name, image, sources.sources().sourcesResolve(
    ...
    sources.sources().play(
        title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, meta, select
    )
    ...
    if rtype == "movie":
        ...
        rlist = episodes.episodes().get(
            tvshowtitle, year, imdb, tvdb, season, create_directory=False
        )
        r = sys.argv[0] + "?action=play"
    elif rtype == "season":
        from resources.lib.indexers import episodes
        ...
        rlist = episodes.seasons().get(
            tvshowtitle, year, imdb, tvdb, create_directory=False
        )
    ...
    try:
        ...
        for p in [
            ...
            if rtype == "show" and p == "tvshowtitle":
                try:
                    r += "&" + p + "=" + \
                        urllib.quote_plus(rlist[rand]["title"])
                except:
                    pass
            else:
                try:
