...
class AddonPlayer(object):
    def __init__(self, filename, media, meta):
        ...
        self.id = meta.get("id", filename.replace(".metallik.json", ""))
...
#    xbmc.log("QQQQQ AUTOS = {0}".format(str(AUTOS)), xbmc.LOGNOTICE)
    ...
    PATCHES = [
		...
		["lib/resources/lib/modules/control.py", "pass", "sys.exit()"]
	]
    if mode == "auto":
        if AUTOS != [[], [], [], []]:
            ADDONS = AUTOS
        else:
            if dialogs.yesno('{0}: Patch'.format(plugin.name), '{0}.[CR]{1} & {2}'.format(_("%s not found") % 'Auto-patches', _("Enable"), _("Continue?"))): 
				return patch("all")
            else:
        ...
        ADDONS = [
			[], 
			[i for i in INSTALLED if i.startswith("plugin.video.") and xbmcvfs.exists("{0}{1}/{2}".format(adir, i, PATCHES[1][0]))], 
			[i for i in INSTALLED if i.startswith("plugin.video.") and xbmcvfs.exists("{0}{1}/{2}".format(adir, i, PATCHES[2][0]))], 
			[i for i in INSTALLED if i.startswith("script.module.") and xbmcvfs.exists("{0}{1}/{2}".format(adir, i, PATCHES[3][0]))],
		]
    count = 0
