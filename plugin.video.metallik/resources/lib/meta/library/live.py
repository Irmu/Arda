...
def setup_library(library_folder):
    if library_folder[-1] != "/":
        library_folder += "/"
    metallik_playlist_folder = "special://profile/playlists/mixed/metallik/"
    if not xbmcvfs.exists(metallik_playlist_folder): xbmcvfs.mkdir(metallik_playlist_folder)
    ...
    if not xbmcvfs.exists(library_folder):
        ...
        msg = _("Would you like to automatically set metallik as a channel video source?")
        if dialogs.yesno("{0} {1}".format(_("Library"), "setup"), msg):
            source_thumbnail = get_icon_path("live")
            source_name = "metallik " + _("Channels")
