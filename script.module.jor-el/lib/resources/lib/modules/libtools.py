...
class libtvshows:
    ...
    def strmFile(self, i):
        try:
            ...
            #f = open('C:\\Users\\Ray\\Dropbox\\GDrive\\tvshows.xml', 'a')
            #f.write('<entry><tvshow>%s</tvshow><filename>%s S%02dE%02d</filename><url></url></entry>\n' % (transtitle,transtitle,int(season), int(episode)))
            #f.close()
