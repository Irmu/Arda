    ...
    def strmFile(self, i):
        try:
            ...
            systitle, syspremiered = (
                urllib.quote_plus(tvshowtitle),
                urllib.quote_plus(premiered),
            )

            transtitle = cleantitle.normalize(tvshowtitle.translate(None, '\/:*?"<>|'))

            content = (
                "%s?action=play&title=%s&year=%s&imdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&date=%s"
                % (
                    sys.argv[0],
                    episodetitle,
                    year,
        ...
        for item in items:
            ...
            try:
                ...
                if status == "continuing":
                    raise Exception()
                dbcur.execute(
                    "INSERT INTO tvshows Values (?, ?)", (item["tvdb"], repr(it))
                )
                dbcon.commit()
            except:
                pass
