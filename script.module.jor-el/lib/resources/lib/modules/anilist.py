...
def _getAniList(url):
    try:
        ...
        return client.request(
            url,
            headers={
                "Authorization": "%s %s" % cache.get(_getToken, 1),
...
def getAlternativTitle(title):
    try:
        ...
        r = [
            (i.get("title_romaji"), i.get("synonyms", []))
            for i in utils.json_loads_as_str(r)
            if cleantitle.get(i.get("title_english", "")) == t
        ]
