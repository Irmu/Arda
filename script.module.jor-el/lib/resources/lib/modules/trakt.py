...
def __getTrakt(url, post=None):
    try:
        ...
        if getTraktCredentialsInfo():
            headers.update(
                {"Authorization": "Bearer %s" % control.setting("trakt.token")}
            )

        result = client.request(
            url, post=post, headers=headers, output="extended", error=True
        )
        ...
        if resp_code in ["500", "502", "503", "504", "520", "521", "522", "524"]:
            ...
            log_utils.log(
                "Trakt Rate Limit Reached: %s" % resp_code, log_utils.LOGWARNING
            )
            return
...
def authTrakt():
    try:
        ...
        verification_url = (control.lang(32513) % result["verification_url"]).encode(
            "utf-8"
        )
        ...
        result = client.request(
            urlparse.urljoin(BASE_URL, "/users/me"), headers=headers
        )
        result = utils.json_loads_as_str(result)
...
def manager(name, imdb, tvdb, content):
    try:
        ...
        select = control.selectDialog(
            [i[0] for i in items], control.lang(32515).encode("utf-8")
        )
