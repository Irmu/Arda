...
V2_API_KEY = '1f17a4f9adfbb465857f598a0d4f9a324f9e85d565ea997904457501c5a9b9d2'
CLIENT_SECRET = '2e2d331a5433bd71ec9b7abb18720f47b466b786adcd7e8e85776097ddac189e'
...
def getTraktAsJson2(url, post=None):
    try:
        r, res_headers = __getTrakt(url, post)
        r = utils.json_loads_as_int(r)
        return r
    except:
        pass
...
def addtocollection(name, imdb, tvdb, content):
    try:
        post = {"movies": [{"ids": {"imdb": imdb}}]} if content == 'movie' else {"shows": [{"ids": {"tvdb": tvdb}}]}

        items = '/sync/collection'
        result = __getTrakt(items, post=post)[0]

        icon = control.infoLabel('ListItem.Icon') if not result == None else 'ERROR'

        control.infoDialog('Added to collection', heading=str(name), sound=True, icon=icon)
    except:
        return

def removefromcollection(name, imdb, tvdb, content):
    try:
        post = {"movies": [{"ids": {"imdb": imdb}}]} if content == 'movie' else {"shows": [{"ids": {"tvdb": tvdb}}]}

        items = '/sync/collection/remove'
        result = __getTrakt(items, post=post)[0]

        icon = control.infoLabel('ListItem.Icon') if not result == None else 'ERROR'

        control.infoDialog('Removed from collection', heading=str(name), sound=True, icon=icon)
    except:
        return

def addtowatchlist(name, imdb, tvdb, content):
    try:
        post = {"movies": [{"ids": {"imdb": imdb}}]} if content == 'movie' else {"shows": [{"ids": {"tvdb": tvdb}}]}

        items = '/sync/watchlist'
        result = __getTrakt(items, post=post)[0]

        icon = control.infoLabel('ListItem.Icon') if not result == None else 'ERROR'

        control.infoDialog('Added to watchlist', heading=str(name), sound=True, icon=icon)
    except:
        return

def removefromwatchlist(name, imdb, tvdb, content):
    try:
        post = {"movies": [{"ids": {"imdb": imdb}}]} if content == 'movie' else {"shows": [{"ids": {"tvdb": tvdb}}]}
        ...
        control.infoDialog('Removed from watchlist', heading=str(name), sound=True, icon=icon)
    except:
        return
...
def addRemoveCollection(name, imdb, tvdb, content):
	import xbmcgui
	select = xbmcgui.Dialog().select('Trakt Collection Options', [
		'Add %s to Trakt Collection' % name,
		'Remove %s from Trakt Collection' % name
	])
	if select == 0:
		addtocollection(name, imdb, tvdb, content)	
	if select == 1:
		removefromcollection(name, imdb, tvdb, content)

def addRemoveWatchlist(name, imdb, tvdb, content):
	import xbmcgui
	select = xbmcgui.Dialog().select('Trakt Watchlist Options', [
		'Add %s to Trakt Watchlist' % name,
		'Remove %s from Trakt Watchlist' % name
	])
	if select == 0:
		addtowatchlist(name, imdb, tvdb, content)
	if select == 1:
		removefromwatchlist(name, imdb, tvdb, content)
