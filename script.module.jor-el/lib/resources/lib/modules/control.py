...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.jor-el.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.jor-el.artwork').getAddonInfo('path'), 'resources', 'media', theme)
...
def artwork():
    execute('RunPlugin(plugin://script.jor-el.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.jor-el.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.jor-el.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
