...
def download(name, image, url):
	...
	try:
		season = re.compile('.+? S(.+?)E.+?').findall(name)
		episode = re.compile('.+? S.+?E(.+?)').findall(name)
	except: pass
	...
	if len(content) == 0:
		...
		for level in levels:
			try: control.makeFile(os.path.abspath(os.path.join(dest)))
			except: pass
		...
		if transtvshowtitle == 'Happy Together': transtvshowtitle = 'Happy Together (2018)'
		if transtvshowtitle == 'Nancy Drew': transtvshowtitle = 'Nancy Drew (2019)'
		if transtvshowtitle == 'Once Upon a Time': transtvshowtitle = 'Once Upon a Time (2011)'
		if transtvshowtitle == 'Once Upon A Time': transtvshowtitle = 'Once Upon a Time (2011)'
		if transtvshowtitle == 'The Flash': transtvshowtitle = 'The Flash (2014)'
		if transtvshowtitle == 'Titans': transtvshowtitle = 'Titans (2018)'
		if transtvshowtitle == 'War of the Worlds': transtvshowtitle = 'War of the Worlds (2019)'
		if transtvshowtitle == 'Whose Line is it Anyway': transtvshowtitle = 'Whose Line Is It Anyway (US)'
...
def doDownload(url, dest, title, image, headers):
	...
	while True:
		...
		if percent >= notify:
			#xbmc.executebuiltin( "XBMC.Notification(%s,%s,%i,%s)" % ( title + ': ' + str(percent)+'%', dest, 10000, image))
			#xbmc.executebuiltin( "XBMC.Notification(%s,%s,%i,%s)" % ( str(percent)+'%', title, 5000, image))
			xbmc.executebuiltin( "XBMC.Notification(%s,%s,%i,%s)" % ( str(percent)+'%', title, 500, image))
