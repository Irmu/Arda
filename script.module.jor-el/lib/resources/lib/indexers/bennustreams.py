...
AddonTitle = "bennu"
PARENTAL_FOLDER = xbmc.translatePath(
    os.path.join("special://home/userdata/addon_data/" + addon_id, "parental")
)
...
class indexer:
    ...
    def private(self):
        ...
        pb_list = [
            base64.b64decode("aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3LyVz") % i
            for i in pb_list
            if not i == "" and len(i) == 8
        ]
        ...
        pattern = """%s""" % base64.b64decode(
            "W2F0PGlzbWw+ZW5dezEwfShbXjxdKylbYVwvdDxpc21sPmVuezExfV0="
        )
        ...
        if skip_m:
            ...
            select = control.selectDialog(
                [i[1] for i in dialog_list], control.addonInfo("name")
            )
    ...
    def parental_controls(self):
        ...
        if os.path.isfile(PARENTAL_FILE):
            choice = dialog.yesno(
                AddonTitle, "Would you like to disable parental controls?"
            )
            if choice:
                vq = client._get_keyboard(
                    heading="Please Enter Your Password", hidden=True
                )
                if not vq:
                ...
                for line in vers:
                    ...
                    for password in file:
                        if not password == pass_one:
                            ...
                            remove_me = 1
                            dialog.ok(
                                AddonTitle, "Parental controls have been disabled."
                            )
            ...
            choice = dialog.yesno(
                AddonTitle,
                "To use the ADULT section you must set a password.",
            ...
            if choice:
                ...
                vq = client._get_keyboard(
                    heading="Please Confirm Your Password", hidden=True
                )
                if not vq:
                    dialog.ok(AddonTitle, "Sorry, no password was entered.")
                    quit()
                ...
                if pass_one == pass_two:
                    ...
                    dialog.ok(
                        AddonTitle, "The passwords do not match, please try again."
                    )
                    quit()
    ...
    def tvtuner(self, url):
        try:
            ...
            url, imdb, tvdb, tvshowtitle, year, thumbnail, fanart = (
                re.findall("<url>(.+?)</url>", url)[0],
            ...
            if tvm == None:
                tvm = client.request(
                    "http://api.tvmaze.com/lookup/shows?imdb=%s" % imdb
                )
            tvm = "http://api.tvmaze.com/shows/%s/episodes" % str(
                json.loads(tvm).get("id")
            )
            ...
            for i in items:
                try:
                    if int(re.sub("[^0-9]", "", str(i[3]))) > today:
                        raise Exception()
                    result += (
                        "<item><title> %01dx%02d . %s</title><meta><content>episode</content><imdb>%s</imdb><tvdb>%s</tvdb><tvshowtitle>%s</tvshowtitle><year>%s</year><title>%s</title><premiered>%s</premiered><season>%01d</season><episode>%01d</episode></meta><link><sublink>search</sublink><sublink>searchsd</sublink></link><thumbnail>%s</thumbnail><fanart>%s</fanart></item>"
                        % (
                            int(i[0]),
                            int(i[1]),
                            i[2],
    ...
    def addSearch(self, url):
        ...
        def search():
            return [
                x
                for y, x in enumerate((query + [input]))
                if x not in (query + [input])[:y]
            ]
        ...
        if section == 0:
            ...
            links = [
                i
                for i in links
                if str(i).startswith("http") and matcher.lower() in str(i).lower()
            ]

        self.list = []
        threads = []
        for link in links:
            threads.append(workers.Thread(self.bennu_list, link))
        ...
        if u:
            ...
            r = [
                (
                    ...
                    dom_parser2.parse_dom(i, "span", {"class": "tMeterScore"}),
                    dom_parser2.parse_dom(i, "img", req="src"),
                    dom_parser2.parse_dom(i, "div", {"class": ["info", "synopsis"]}),
                )
                for i in u
            ]
            r = [
                ...
                try:
                    name = "%s (%s)" % (
                        client.replaceHTMLCodes(
                            client.removeNonAscii(i[0].encode("utf-8"))
                        ),
                        i[1],
                    )
                    original_name = client.replaceHTMLCodes(
                        client.removeNonAscii(i[0].encode("utf-8"))
                    )

                    url = (
                        "<sublink><preset>search</preset>        <content>movie</content>        <imdb>%s</imdb>        <title>%s</title>        <year>%s</year>        </sublink>        <sublink><preset>searchsd</preset>        <content>movie</content>        <imdb>%s</imdb>        <title>%s</title>        <year>%s</year>        </sublink>        <thumbnail>%s</thumbnail>"
                    ...
                    self.list.append(
                        {
                            ...
                            "worker": "0",
                            "rating": i[2],
                            "plot": client.replaceHTMLCodes(i[4]),
                        }
                    )
                except:
                    log_utils.log(
                        "Scraping Error: Could not add %s to directory."
                        % str(original_name),
                        log_utils.LOGERROR,
                    )
            return self.list
    ...
    def trakt_list(self, url):
        ...
        r = [
            (
                i.attrs["data-runtime"],
                dom_parser2.parse_dom(i, "img", {"class": "real"}),
            )
            for i in u
        ]
        r = [
            (
                i[1][0].attrs["title"],
                re.findall("\(([^\)]+)", i[1][0].attrs["title"])[0]
                if "(" in i[1][0].attrs["title"]
                else "0",
                i[0],
                i[1][0].attrs["data-original"],
            )
            for i in r
            if i[0] and i[1]
        ]
    ...
    def imdb_list(self, url):
        ...
        try:
            ...
            if not np.startswith("http"):
                np = urlparse.urljoin(base_url, np)
        ...
        r = [
            (
                dom_parser2.parse_dom(i, "div", {"class": "hover-over-image"}),
                dom_parser2.parse_dom(i, "a", req="href"),
        ...
        r = [
            (
                i[0][0].attrs["data-const"],
                i[1][1].content,
                re.findall("(\d+)", i[2][0].content)[0],
                re.sub("<.+?>", "", i[3][0].content),
                i[4][0].attrs["loadlate"]
                if "loadlate=" in i[4][0].content
                else i[4][-1].attrs["src"],
    ...
    def bennu_list(self, url, result=None, enc=False):
        ...
        try:
            ...
            if result == None:
                result = cache.get(client.request, 1, url)
            ...
            if result.strip().startswith("#EXTM3U") and "#EXTINF" in result:
                ...
                result = [
                    "<item><title>%s</title><link>%s</link></item>" % (i[0], i[1])
                    for i in result
                ]
                result = "".join(result)
        ...
        for item in items:
            try:
                regdata = re.compile(
                    "(<regex>.+?</regex>)", re.MULTILINE | re.DOTALL
                ).findall(item)
                regdata = "".join(regdata)
                reglist = re.compile(
                    "(<listrepeat>.+?</listrepeat>)", re.MULTILINE | re.DOTALL
                ).findall(regdata)
                ...
                if "tvshow" in content and not url.strip().endswith(".xml"):
                    url = (
                        "<preset>tvindexer</preset><url>%s</url><thumbnail>%s</thumbnail><fanart>%s</fanart>%s"
                ...
                if "tvtuner" in content and not url.strip().endswith(".xml"):
                    url = (
                        "<preset>tvtuner</preset><url>%s</url><thumbnail>%s</thumbnail><fanart>%s</fanart>%s"
                        % (url, image2, fanart2, meta)
                    )
                    action = "tvtuner"
                ...
                try:
                    rotten_list = re.findall("<rotten_list>(.+?)</rotten_list>", item)[
                        0
                    ]
                except:
                    rotten_list = False
                ...
                try:
                    tvshowtitle = re.findall("<tvshowtitle>(.+?)</tvshowtitle>", meta)[
                        0
                    ]
                except:
                    tvshowtitle = "0"
    ...
    def worker(self):
        ...
        self.fanart_tv_headers = {
            "api-key": "NDZkZmMyN2M1MmE0YTc3MjY3NWQ4ZTMyYjdiY2E2OGU=".decode("base64")
        }
        ...
        for r in range(0, total, 50):
            ...
            pDialog.update(
                percent, "MetaData Progress", "Retrieving MetaData Information"
            )
    ...
    def movie_info(self, i):
        try:
            ...
            try:
                ...
                self.tm_art_link = (
                    "http://api.themoviedb.org/3/movie/%s/images?api_key=%s&language=en-US&include_image_language=en,%s,null"
            ...
            try:
                ...
                poster3 = (
                    ...
                    + [
                        x
                        for x in poster3
                        if x.get("iso_639_1") not in [self.lang, "en"]
                    ]
                )
                poster3 = [(x["width"], x["file_path"]) for x in poster3]
                poster3 = [
                    (x[0], x[1]) if x[0] < 300 else ("300", x[1]) for x in poster3
                ]
            ...
            try:
                ...
                fanart2 = [x for x in fanart2 if x.get("width") == 3840] + [
                    x for x in fanart2 if x.get("width") < 3840
                ]
                fanart2 = [(x["width"], x["file_path"]) for x in fanart2]
                fanart2 = [
                    (x[0], x[1]) if x[0] < 1280 else ("1280", x[1]) for x in fanart2
                ]
    ...
    def addDirectory(self, items, queue=False, cache=True):
        ...
        try:
            devmode = (
                True
                if "testings.xml" in control.listDir(control.dataPath)[1]
                else False
            )
        except:
            devmode = False
        ...
        for i in items:
            try:
                ...
                if build:
                    ...
                    try:
                        devurl = dict(urlparse.parse_qsl(urlparse.urlparse(url).query))[
                            "action"
                        ]
                    except:
                        devurl = None
                    ...
                    try:
                        ...
                        privurl = None
                    if privurl == "private" and not privmode == True:
                        raise Exception()
                    ...
                    if fanart == "0":
                        try:
                            fanart = (
                                i["fanart"] if not "fanart.tv" in i["fanart"] else "0"
                            )
                        except:
                            fanart = "0"
                    ...
                    if poster == "0":
                        ...
                        poster = re.sub(
                            "([a-zA-Z]+)\d+_([a-zA-Z]+)\d+,\d+,\d+,\d+",
                            "\g<1>512_\g<2>0,0,0,512",
                            poster,
                        )
                    ...
                    if content in ["movies", "tvshows"]:
                        ...
                        cm.append(
                            (
                                control.lang(30707).encode("utf-8"),
                    ...
                    if content in ["movies", "tvshows", "seasons", "episodes"]:
                        cm.append(
                            (control.lang(30708).encode("utf-8"), "XBMC.Action(Info)")
                        )
                    ...
                    if (folder == False and not "|regex=" in str(i.get("url"))) or (
                        ...
                        cm.append(
                            (
                                control.lang(30723).encode("utf-8"),
                    ...
                    if content == "movies":
                        ...
                        try:
                            cm.append(
                                (
                                    control.lang(30722).encode("utf-8"),
                                    "RunPlugin(%s?action=addDownload&name=%s&url=%s&image=%s)"
                                    % (
                                        sysaddon,
                                        urllib.quote_plus(dfile),
                    ...
                    if mode == "movies":
                        ...
                        cm.append(
                            (
                                control.lang(30712).encode("utf-8"),
                        ...
                        cm.append(
                            (
                                control.lang(30713).encode("utf-8"),
                    ...
                    item = control.item(
                        label=name, iconImage=poster, thumbnailImage=poster
                    )
        ...
        try:
            ...
            control.addItem(
                handle=int(sys.argv[1]), url=url, listitem=item, isFolder=True
            )
        except:
            pass
...
class resolver:
    def browser(self, url):
        try:
            ...
            if url == False:
                return
            control.execute(
                "RunPlugin(plugin://plugin.program.chrome.launcher/?url=%s&mode=showSite&stopPlayback=no)"
        ...
        try:
            ...
            for i in items:
                if i[0] != "":
                    ...
                    dialog_list += [
                        ("SD (Lists provider, host & quality if available)", i[1])
                    ]
                elif "<preset>search</preset>" in i[1]:
                    dialog_list += [
                        (
                            "AUTOPLAY (Best available stream - can be false at times)",
                            i[1].replace(
                                "<preset>search</preset>", "<preset>searchauto</preset>"
                            ),
                        )
                    ]
                    if not debrid.status() == False:
                        dialog_list += [
                            (
                                "Premium (Real Debrid, Premiumize, etc)",
                                i[1].replace(
                                    "<preset>search</preset>",
                                    "<preset>searchrd</preset>",
                                ),
                            )
                        ]
                    dialog_list += [
                        ("HD (Lists provider, host & quality if available)", i[1])
                    ]

            select = control.selectDialog(
                [i[0] for i in dialog_list], control.infoLabel("listitem.label")
            )
    ...
    def f4m(self, url, name):
        try:
            ...
            return f4mProxyHelper().playF4mLink(
                url,
                name,
                proxy,
                proxy_use_chunks,
    ...
    def process(self, url, direct=True):
        ...
        try:
            ...
            try:
                tvdb, tvshowtitle, premiered, season, episode = (
                    re.findall("<tvdb>(.+?)</tvdb>", url)[0],
                    re.findall("<tvshowtitle>(.+?)</tvshowtitle>", url)[0],
...
class player(xbmc.Player):
    ...
    def play(self, url, content=None):
        try:
            ...
            self.getbookmark = (
                True if (content == "movies" or content == "episodes") else False
            )
...
class bookmarks:
        ...
        try:
            ...
            try:
                yes = control.dialog.contextmenu(
                    [label, control.lang(32501).encode("utf-8")]
                )
            except:
    ...
    def reset(self, currentTime, totalTime, name, year="0"):
        try:
            ...
            ok = int(currentTime) > 180 and (currentTime / totalTime) <= 0.92
            ...
            if ok:
                dbcur.execute(
                    "INSERT INTO bookmark Values (?, ?)", (idFile, timeInSeconds)
                )
            dbcon.commit()
        except:
            pass
