...
class navigator:
    ...
    NEWSFILE      = base64.b64decode(b'aHR0cDovL2tvZGloZWxwLm5ldC90ZXN0L1VwZGF0ZXMudHh0')
    LOCALNEWS     = os.path.join(THISADDONPATH, 'Updates.txt')
    TORRENTSFILE  = base64.b64decode(b'aHR0cDovL2tvZGloZWxwLm5ldC90ZXN0L1VwZGF0ZXMudHh0')
    TORRENTSLOCAL = os.path.join(THISADDONPATH, 'torrents.txt')    
    def root(self):
        self.addDirectoryItem('[COLOR=red]Jor-El NEWS[/COLOR]', 'newsNavigator', 'tools.png', 'DefaultAddonProgram.png')
...
#        self.addDirectoryItem(32540, 'Resolveurl', 'resolveurl.png', 'DefaultAddonProgram.png')
    ...
    def news(self):
            ...
            if len(message)>1:
                    if compfile == message:pass
                            ...
                            text_file.close()
                            compfile = message
            self.showText('[B][COLOR red]Jor-El NEWS[/COLOR][/B]', compfile)
...
# Whats New Code
    def torrent(self):
            message=self.open_torrents_url(self.TORRENTSFILE)
            r = open(self.LOCALNEWS)
            compfile = r.read()       
            if len(message)>1:
                    if compfile == message:pass
                    else:
                            text_file = open(self.TORRENTSFILE, "w")
                            text_file.write(message)
                            text_file.close()
                            compfile = message
            self.showText('[B][COLOR red]Setup Torrents in Jor-El[/COLOR][/B]', compfile)
...
#   def Resolver(self):
#      self.addDirectoryItem(32608, 'resolveurl.png', 'DefaultAddonProgram.png')
#
#   	self.endDirectory()        
