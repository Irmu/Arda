...
params = (
    dict(urlparse.parse_qsl(sys.argv[2].replace("?", "")))
    if len(sys.argv) > 1
    else dict()
)
...
class tvshows:
    def __init__(self):
        ...
        self.personlist_link = (
            "http://www.imdb.com/search/name?count=100&gender=male,female"
        )
        ...
        self.imdblists_link = (
            "http://www.imdb.com/user/ur%s/lists?tab=all&sort=mdfd&order=desc&filter=titles"
            % self.imdb_user
        )
        ...
        self.imdbwatchlist_link = (
            "http://www.imdb.com/user/ur%s/watchlist?sort=alpha,asc" % self.imdb_user
        )
        self.imdbwatchlist2_link = (
            "http://www.imdb.com/user/ur%s/watchlist?sort=date_added,desc"
            % self.imdb_user
        )
    ...
    def search(self):
        navigator.navigator().addDirectoryItem(
            32603, "tvSearchnew", "search.png", "DefaultTVShows.png"
        )
    ...
    def browse(self):
        ...
        match = re.compile(
            '<a href="/search/keyword\?keywords=(.+?)&.+?" > (.+?) </a>'
        ).findall(r)
        for keyword, name in match:
            import string
            ...
            self.list.append(
                {
                    "name": name,
                    "url": self.keyword_link % (keyword, self.list_items),
                    "image": "tags.png",
                    "action": "tvshows",
                }
            )
    ...
    def findKeywords(self, url):
        url = "https://www.imdb.com/title/%s/keywords" % url
        r = client.request(url)
        # match = re.compile('<a href="/search/keyword\?keywords=(.+?)&.+?"\n>(.+?)</a>').findall(r)
        match = re.compile(
            '<a href="/search/keyword\?keywords=(.+?)&.+?"\n>(.+?)</a>'
        ).findall(r)
        for keyword, name in match:
            import string
        ...
        cm.append(
            (
                "Browse tags test",
                "ActivateWindow(10025,%s?action=findKeywordsTV&url=me,return)"
                % sysaddon,
            )
        )
        self.addDirectory(self.list)
    ...
    def networks(self):
        networks = [
            ...
            (
                "Adult Swim",
                "/networks/10/adult-swim",
                "https://i.imgur.com/jCqbRcS.png",
            ),
            ("Amazon", "/webchannels/3/amazon", "https://i.imgur.com/ru9DDlL.png"),
            (
                "Animal Planet",
                "/networks/92/animal-planet",
                "https://i.imgur.com/olKc4RP.png",
            ),
            (
                "Audience",
                "/networks/31/audience-network",
                "https://i.imgur.com/5Q3mo5A.png",
            ),
            (
                "BBC America",
                "/networks/15/bbc-america",
                "https://i.imgur.com/TUHDjfl.png",
            ),
            ("BBC Four", "/networks/51/bbc-four", "https://i.imgur.com/PNDalgw.png"),
            ("BBC One", "/networks/12/bbc-one", "https://i.imgur.com/u8x26te.png"),
            (
                "BBC Three",
                "/webchannels/71/bbc-three",
                "https://i.imgur.com/SDLeLcn.png",
            ),
            ...
            (
                "Cartoon Network",
                "/networks/11/cartoon-network",
                "https://i.imgur.com/zmOLbbI.png",
            ),
            ...
            (
                "Comedy Central",
                "/networks/23/comedy-central",
                "https://i.imgur.com/ko6XN77.png",
            ),
            ("Crackle", "/webchannels/4/crackle", "https://i.imgur.com/53kqZSY.png"),
            (
                "Discovery Channel",
                "/networks/66/discovery-channel",
                "https://i.imgur.com/8UrXnAB.png",
            ),
            (
                "Discovery ID",
                "/networks/89/investigation-discovery",
                "https://i.imgur.com/07w7BER.png",
            ),
            (
                "Disney Channel",
                "/networks/78/disney-channel",
                "https://i.imgur.com/ZCgEkp6.png",
            ),
            ...
            (
                "Hallmark",
                "/networks/50/hallmark-channel",
                "https://i.imgur.com/zXS64I8.png",
            ),
            (
                "History Channel",
                "/networks/53/history",
                "https://i.imgur.com/LEMgy6n.png",
            ),
            ...
            (
                "National Geographic",
                "/networks/42/national-geographic-channel",
                "https://i.imgur.com/XCGNKVQ.png",
            ),
            ("Netflix", "/webchannels/1/netflix", "https://i.imgur.com/jI5c3bw.png"),
            (
                "Nickelodeon",
                "/networks/27/nickelodeon",
                "https://i.imgur.com/OUVoqYc.png",
            ),
            ...
            (
                "Travel Channel",
                "/networks/82/travel-channel",
                "https://i.imgur.com/mWXv7SF.png",
            ),
    ...
    def imdb_list(self, url):
        ...
        try:
            ...
            next = url.replace(
                urlparse.urlparse(url).query, urlparse.urlparse(next[0]).query
            )
    ...
    def tvmaze_list(self, url):
        try:
            ...
            result = client.parseDOM(
                result, "section", attrs={"id": "this-seasons-shows"}
            )
    ...
    def worker(self, level=1):
        ...
        self.fanart_tv_headers = {
            "api-key": "ZDY5NTRkYTk2Yzg4ODFlMzdjY2RkMmQyNTlmYjk1MzQ=".decode("base64")
        }
        ...
        try:
            ...
            if tvdb == "0":
                ...
                tvdb = [
                    x
                    for x in tvdb
                    if cleantitle.get(self.list[i]["title"]) == cleantitle.get(x[1])
                ]
            ...
            try:
                ...
                fanart2 = (
                    [x for x in fanart2 if x.get("lang") == self.lang][::-1]
                    + [x for x in fanart2 if x.get("lang") == "en"][::-1]
                    + [x for x in fanart2 if x.get("lang") in ["00", ""]][::-1]
                )
            ...
            try:
                ...
                banner2 = (
                    [x for x in banner2 if x.get("lang") == self.lang][::-1]
                    + [x for x in banner2 if x.get("lang") == "en"][::-1]
                    + [x for x in banner2 if x.get("lang") in ["00", ""]][::-1]
                )
    ...
    def tvshowDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                if flatten == True:
                    url = (
                        "%s?action=episodes&tvshowtitle=%s&year=%s&imdb=%s&tvdb=%s"
