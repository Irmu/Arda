...
class source:
    def __init__(self):
        ...
        self.search_link = (
            "/torrents-search.php?search=%s&sort=seeders&order=desc&page=%s"
        )
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            try:  # Page 1
                ...
                # xbmcgui.Dialog().textviewer('Check',r)
                file = xbmc.translatePath(
                    "special://userdata/addon_data/plugin.video.jor-el/html.txt"
                )
                f = open(file, "w")
                f.write(r)
                f.close()
                match = re.compile('class="showthecross"><a href="(.+?)&amp').findall(r)
                for url in match:
                    url = self.base_link + "/" + url
                    r = client.request(url)
                    match = re.compile(
                        "<td class='tabledata1'>Info Hash:</td><td class='tabledata0'>(.+?)</td>"
                    ).findall(r)
                    for hash in match:
                        # Find infohash
                        url = (
