...
class source:
    ...
    def movie(self, imdb, title, localtitle, aliases, year):
        if xbmc.getCondVisibility("System.HasAddon(script.realdebrid.mod)"):
            try:
                ...
                url = (
                    self.base_link
                    + "/search_results.php?search=%s+%s&cat=1&incldead=0&inclexternal=0&lang=0&sort=seeders&order=desc"
                    % (title, year)
                )
                return url
            except:
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            url = (
                self.base_link
                + "/search_results.php?search=%s+s%se%s&cat=41&incldead=0&inclexternal=0&lang=0&sort=seeders&order=desc"
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            try:  # Page 1
                ...
                match = re.compile(
                    "<b>(.+?)</b></a></td><td class='ttable_col1' align='center'><a href= http://itorrents\.org/torrent/(.+?)\.torrent"
                ).findall(r)
