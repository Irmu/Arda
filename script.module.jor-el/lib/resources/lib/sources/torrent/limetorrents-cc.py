...
class source:
    def __init__(self):
        ...
        self.base_link = "https://limetorrents.info"
        self.search_link = "/search/all/%s/seeds/%s/"
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            try:  # Page 1
                ...
                match = re.compile(
                    '<div class="tt-name"><a href="http://itorrents\.org/torrent/(.+?)\.torrent\?title=.+?" rel="nofollow" class="csprite_dl14"></a><a href="(.+?)">(.+?)</a>'
                ).findall(r)
                for hash, url, info in match:
                    url = (
