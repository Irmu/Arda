...
class source:
    def __init__(self):
        ...
        # self.base_link = 'http://1377x.to'
        self.base_link = "https://1377x.to"
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            try:  # Page 1
                url = self.base_link + "/search/" + q + "/1/"
                r = client.request(url)
                match = re.compile('<a href="/torrent/(.+?)">(.+?)</a>').findall(r)
                for url, filename in match:
                    torrenturl = "https://1337x.to/torrent/%s" % url
