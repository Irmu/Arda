...
class MagnetDL(SearcherABC.SearcherABC):
    ...
    baseurl = 'magnetdl.com' # TODO: change this!
    ...
    def query(self, keyword):
        keyword=self.unescape(self.stripHtml(keyword)).replace(' ','-').lower()
        self.debug(keyword)
        return keyword

    def search(self, keyword):
        filesList = []
        sort="se" if self.__settings__.getSetting('sortby')=='0' else "age"
        url = "http://%s/%s/%s/%s/desc/" % (self.baseurl, self.query(keyword)[:1], self.query(keyword), sort)
        headers = [('User-Agent',
        ...
        if None != response and 0 < len(response):
            ...
            regex_tr='''<tr><td class="m"><a href="(magnet.+?)".+?title=".+?" rel="nofollow">.+?<a href=".+?" title="(.+?)">.+?</td><td>.+?</td><td class=".+?">(.+?)</td><td>.+?</td><td>(.+?)</td><td class="s">(\d+)</td><td class="l">(\d+)</td>'''
