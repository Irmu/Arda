...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.pttp/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.pttp/resources/catchup', 'g'))
...
def start():
	if username=="":
		...
		if auth == "":
			...
			line2 = "Welcome to Power to The People" 
			line3 = ('[COLORwhite]%s[/COLOR]'%user)
			xbmcgui.Dialog().ok('Power to The People', line1, line2, line3)
			tvguidesetup()
			addonsettings('ADS2','')
			xbmc.executebuiltin('Container.Refresh')
			home()
		...
		if not auth=="":
			...
			tools.addDir('[COLORwhite]Live Tv[/COLOR]','live',1,icon,fanart,'')
...
def home():
	...
	tools.addDir('Series',Series_url,3,icon,fanart,'')
...
def catchup():
    loginurl   = "http://crystaltv.ddns.net:8080/get.php?username=" + username + "&password=" + password + "&type=m3u_plus&output=ts"
    try:
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            ...
            dialog.ok("Power to The People", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search Power to The People:')
	if search=="":
		return
	else:
		return search
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('Power to The People', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('Power to The People', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok('Power to The People', 'iVue Integration Complete')
		elif dialog==1:
			pvrsetup()
			xbmcgui.Dialog().ok('Power to The People', 'PVR Integration Complete')
		elif dialog==2:
			pvrsetup()
			ivueint()
			xbmcgui.Dialog().ok('Power to The People', 'PVR & iVue Integration Complete')
	elif url =="ST":
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.pttp/resources/modules/speedtest.py")')
	elif url =="META":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('Power to The People', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def correctPVR():
	...
	addon = xbmcaddon.Addon('plugin.video.pttp')
	...
	loginurl   = "http://crystaltv.ddns.net:8080/get.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
	EPGurl     = "http://crystaltv.ddns.net:8080/xmltv.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('Power to The People','Would You like us to Setup the TV Guide for You?')
		if dialog:
			dialog = xbmcgui.Dialog().select('Select a TV Guide to Setup', ['iVue TV Guide','PVR TV Guide','Both'])
			if dialog==0:
				ivueint()
