...
class sources:
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, quality='HD', timeout=30):
        ...
        for i in range(0, 4 * timeout):
            ...
            try:
                ...
                if len(self.sources) > 0:
                    if quality in ['0']:
                        ...
                        source_1080 = len([e for e in self.sources if e['quality'] in [
                                          '1440p', '1080p'] and e['debridonly'] == False])
                        source_720 = len([e for e in self.sources if e['quality'] in [
                                         '720p', 'HD'] and e['debridonly'] == False])
                        ...
                        source_1080 = len([e for e in self.sources if e['quality']
                                           in ['1080p'] and e['debridonly'] == False])
                        source_720 = len([e for e in self.sources if e['quality'] in [
                                         '720p', 'HD'] and e['debridonly'] == False])
                        source_sd = len([e for e in self.sources if e['quality'] == 'SD' and e['debridonly'] == False])
                    elif quality in ['3']:
                            ...
                            for d in debrid_list:
                                ...
                                d_source_1080 = len(
                                    [e for e in self.sources
                                     if e['quality'] in['1440p', '1080p'] and d.valid_url(
                                         str(e['url']),
                                         e['source'])])
                ...
                if (i / 2) < timeout:
                    try:
                        mainleft = [sourcelabelDict[x.getName()] for x in threads if x.is_alive() ==
                        ...
                        if debrid_status:
                            if len(info) > 6:
                                ...
                                line3 = string3 % (
                                    ', '.join(info).replace(
                                        '[COLOR %s]' % (control.setting('orion.color').upper()),
                                        '').replace('[/COLOR]', ''))
                            else:
                                break
    ...
    def sourcesFilter(self):
        ...
        if not HEVC == 'true':
            self.sources = [i for i in self.sources if not any(value in str(i['url']).lower() for value in [
...
#        self.sources = filter
        ...
        if quality in ['0', '1']:
            filter += [i for i in self.sources if i['quality'] ==
                       '1440p' and not 'debrid' in i and not 'memberonly' in i]
        ...
        if quality in ['0', '1', '2']:
            filter += [i for i in self.sources if i['quality'] ==
                       '1080p' and not 'debrid' in i and not 'memberonly' in i]
        ...
        if quality in ['0', '1', '2', '3']:
            filter += [i for i in self.sources if i['quality'] ==
                       '720p' and not 'debrid' in i and not 'memberonly' in i]
        ...
        for i in range(len(self.sources)):
            ...
            if double_line:
                ...
                if not d == '':
                    ...
                    if multi == True and not l == 'en':
                        label += ' | [B]%s[/B]' % l
                    label += ' | %s | [B]%s[/B] | %s | [B]%s[/B] | [/COLOR][COLOR %s][I]%s /%s[/I][/COLOR]' % (
                        d, q, p, s, sec_identify, f, t)
                    ...
                    if multi == True and not l == 'en':
                        label += ' | [B]%s[/B]' % l
                    label += ' | [B]%s[/B] | %s | [B]%s[/B] | [COLOR %s][I]%s /%s[/I][/COLOR]' % (
                        q, p, s, sec_identify, f, t)

            label = label.replace(' |  |', ' |').replace('| 0 |', '|').replace(
                '[I] /[/I]', '').replace('[I] /%s[/I]' % t, '[I]%s[/I]' % t).replace('[I]%s /[/I]' % f, '[I]%s[/I]' % f)
    ...
    def sourcesResolve(self, item, info=False):
        try:
            ...
            if url == None or (not '://' in url and not local and 'magnet:' not in url):
                raise Exception()
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.marauder.container.items'

        self.metaProperty = 'plugin.video.marauder.container.meta'
        ...
        sourceDir2 = sources()
        from barnaclescrapers import sources
        sourceDir3 = sources()
        try:
        ...
        try:
            if scraperSetting == 'Barnacle Scrapers':
                self.sourceDict = sourceDir3
                self.module_name = 'BarnacleScrapers (' + str(control.addon('script.module.barnaclescrapers').getSetting(
            ...
            elif scraperSetting == 'Barnacle + Built-in':
                self.sourceDict = sourceDir2 + sourceDir3
                self.module_name = 'Built-in + BarnacleScrapers (' + str(control.addon(
                    'script.module.barnaclescrapers').getSetting('module.provider')) + ' module):'
                ...
                self.module_name = 'BarnacleScrapers (' + str(control.addon('script.module.barnaclescrapers').getSetting(
                    'module.provider')) + ' module):'
                control.setSetting('module.provider', 'Barnacle Scrapers')
        except:
            return
